...
import {
  ArrowUp, ExternalLink, ImagePlus, LoaderCircle, MessageSquare, Plus, Sparkles, Trash2, X,
} from "lucide-react";
...
const MODELS = [
  ...
  { value: "gpt-5-5", label: "GPT-5.5" }, { value: "gpt-5-5-mini", label: "GPT-5.5 Mini" },
...
interface MessagePart { type: "text" | "image_gen"; content: string; payload?: any; }
    ...
    for (let i = match.index; i < content.length; i++) {
      ...
      if (!inString) {
        if (char === '{') braceCount++;
          ...
          braceCount--;
          if (braceCount === 0) { foundEnd = i; break; }
        }
      }
    }
    
    if (foundEnd !== -1) {
      const possibleJson = content.slice(match.index, foundEnd + 1);
      try {
        const parsed = JSON.parse(possibleJson);
        if (parsed && typeof parsed === "object" && typeof parsed.prompt === "string") {
          let textBefore = content.slice(lastIndex, match.index);
          textBefore = textBefore.replace(/```(?:json)?\s*$/, '');
          if (textBefore) parts.push({ type: "text", content: textBefore });
          
          parts.push({ type: "image_gen", content: possibleJson, payload: parsed });
          
          lastIndex = foundEnd + 1;
          const afterMatch = content.slice(lastIndex).match(/^\s*```/);
          if (afterMatch) lastIndex += afterMatch[0].length;
        }
      } catch (e) {
...
function ImageGenWidget({ payload }: { payload: any }) {
  ...
  const handleGoToStudio = () => {
    try {
      const data = { prompt: payload.prompt, url: payload.init_image || "" };
      window.sessionStorage.setItem("chatgpt2api:redraw_handoff", JSON.stringify(data));
      router.push("/image");
  ...
  const handleInlineGenerate = async () => {
    ...
    try {
      const res = await generateImage(payload.prompt, "gpt-image-2" as any, payload.size);
      if (res && res.data && res.data[0]) {
        const d = res.data[0];
        setGeneratedUrl(d.b64_json ? `data:image/png;base64,${d.b64_json}` : d.url);
      } else {
        toast.error("生成失败，未返回图像数据");
      }
    } catch (e: any) {
      toast.error(e.message || "生成出错");
    } finally {
  ...
  return (
    <div className="my-3 overflow-hidden rounded-xl border border-violet-500/20 bg-violet-50/50 dark:bg-violet-950/20 shadow-sm">
      <div className="flex items-center gap-2 border-b border-violet-500/10 bg-violet-500/5 px-4 py-2.5">
        <Sparkles className="size-4 text-violet-600" />
        <span className="text-sm font-semibold text-violet-700 dark:text-violet-400">图像生成向导</span>
      </div>
      <div className="p-4 space-y-3">
        <p className="text-[13px] text-foreground/80 leading-relaxed break-words">{payload.prompt}</p>
        {payload.init_image && (
          <div className="flex items-center gap-2 text-[12px] text-muted-foreground bg-background/50 rounded-lg p-2 border border-border/50">
            <ImagePlus className="size-3.5 shrink-0" />
            <span className="truncate">包含底图，建议前往高级画板以启用重绘模式</span>
          </div>
        )}
        {generatedUrl && (
          <div className="mt-3"><img src={generatedUrl} alt="generated" className="w-full max-w-[512px] rounded-lg shadow-sm border border-border" /></div>
        )}
        <div className="flex flex-wrap items-center gap-2 pt-2">
          {!generatedUrl && (
            <Button size="sm" variant="outline" className="h-8 gap-1.5 border-violet-200 text-violet-700 hover:bg-violet-100 hover:text-violet-800 dark:border-violet-800 dark:text-violet-300 dark:hover:bg-violet-900" onClick={handleInlineGenerate} disabled={isGenerating}>
              {isGenerating ? <LoaderCircle className="size-3.5 animate-spin" /> : <Sparkles className="size-3.5" />} 直接生成
            </Button>
          )}
          <Button size="sm" variant="default" className="h-8 gap-1.5 bg-violet-600 hover:bg-violet-700 text-white" onClick={handleGoToStudio}>
            前往高级画板编辑 <ExternalLink className="size-3.5" />
          </Button>
        </div>
      </div>
    </div>
                    ...
                    {msg.loading ? (
                      ...
                      <div className="space-y-2">
                        {parseMessageContent(msg.content).map((part, i) =>
                          part.type === "image_gen" ? (
                            <ImageGenWidget key={i} payload={part.payload} />
                          ) : (
                            <div key={i} className="markdown-body"><ReactMarkdown remarkPlugins={[remarkGfm]}>{part.content}</ReactMarkdown></div>
                          )
                        )}
                      </div>
