...
export function RegisterSettingsCard() {
  ...
  const updateProviderType = (index: number, type: string) => {
    updateProvider(index, {
      ...
      ...(type === "mail.tm" ? {} : {}),
      ...(type === "mail.cx" ? {} : {}),
      ...(type === "imap" ? { imap_server: "imap.qq.com", imap_port: 993, imap_user: "", imap_password: "", domain: [] } : {}),
    });
  };
        ...
        <section className="rounded-xl border border-border bg-card p-5 shadow-sm xl:col-span-1 lg:col-span-2">
          ...
          <div className="space-y-3 max-h-[600px] overflow-y-auto pr-1 scrollbar-fancy">
            ...
            {providers.map((provider, index) => {
              const type = String(provider.type || "1secmail");
                  ...
                  <div className="space-y-1.5">
                    ...
                    >
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="1secmail">1secmail (免配)</SelectItem>
                        <SelectItem value="mail.tm">mail.tm (免配)</SelectItem>
                        <SelectItem value="mail.cx">mail.cx (免配)</SelectItem>
                        <SelectItem value="imap">imap (QQ邮箱转发)</SelectItem>
                      </SelectContent>
                    </Select>
                  </div>
                  ...
                  {type === "imap" && (
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1.5">
                        ...
                        <Input
                          value={String(provider.imap_server || "")}
                          onChange={(event) => updateProvider(index, { imap_server: event.target.value })}
                      ...
                      <div className="space-y-1.5">
                        ...
                        <Input
                          value={String(provider.imap_port || "")}
                          onChange={(event) => updateProvider(index, { imap_port: Number(event.target.value) || 993 })}
                          placeholder="993"
                          className={inputClass}
                          disabled={config.enabled}
                        />
                      ...
                      <div className="space-y-1.5">
                        <label className={labelClass}>IMAP User (Email)</label>
                        <Input
                          value={String(provider.imap_user || "")}
                          onChange={(event) => updateProvider(index, { imap_user: event.target.value })}
                          className={inputClass}
                          disabled={config.enabled}
                        />
                      </div>
                      <div className="space-y-1.5">
                        <label className={labelClass}>IMAP Password</label>
                        <Input
                          value={String(provider.imap_password || "")}
                          onChange={(event) => updateProvider(index, { imap_password: event.target.value })}
                          type="password"
                          className={inputClass}
                          disabled={config.enabled}
                        />
