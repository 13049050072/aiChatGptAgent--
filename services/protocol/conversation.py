...
async def iter_conversation_payloads_async(payloads: AsyncIterable[str], history_text: str = "",
                               history_messages: list[str] | None = None) -> AsyncIterator[dict[str, Any]]:
    state = ConversationState()
    history_messages = history_messages or []
    history_index = 0
...
async def conversation_events_async(
    backend: OpenAIBackendAPI,
    messages: list[dict[str, Any]] | None = None,
    model: str = "auto",
    prompt: str = "",
    ...
    history_messages = [] if image_model else assistant_history_messages(normalized)
    final_prompt = prompt_with_global_system(build_image_prompt(prompt, size)) if image_model else prompt
    payloads = backend.stream_conversation_async(
        messages=normalized,
        model=model,
        prompt=final_prompt,
        images=images if image_model else None,
    ...
    async for event in iter_conversation_payloads_async(payloads, history_text, history_messages):
        yield event


async def stream_text_deltas_async(backend: OpenAIBackendAPI, request: ConversationRequest) -> AsyncIterator[str]:
    attempted_tokens: set[str] = set()
    token = getattr(backend, "access_token", "")
    emitted = False
    while True:
        ...
        try:
            active_backend = OpenAIBackendAPI(access_token=token)
            async for event in conversation_events_async(active_backend, messages=request.messages, model=request.model, prompt=request.prompt):
                if event.get("type") != "conversation.delta":
                    continue
                delta = str(event.get("delta") or "")
                if delta:
            ...
            await run_in_threadpool(account_service.mark_text_used, token)
            return
        except Exception as exc:
            error_message = str(exc)
            if token and not emitted and is_token_invalid_error(error_message):
                from fastapi.concurrency import run_in_threadpool
                await run_in_threadpool(account_service.remove_invalid_token, token, "text_stream")
                from fastapi.concurrency import run_in_threadpool
                token = await run_in_threadpool(account_service.get_text_access_token, attempted_tokens)
                if token:
                    continue
            raise

async def collect_text_async(backend: OpenAIBackendAPI, request: ConversationRequest) -> str:
    parts = []
    async for d in stream_text_deltas_async(backend, request):
        parts.append(d)
    return "".join(parts)


async def stream_image_outputs_async(
        backend: OpenAIBackendAPI,
        request: ConversationRequest,
        index: int = 1,
        total: int = 1,
) -> AsyncIterator[ImageOutput]:
    last: dict[str, Any] = {}
    async for event in conversation_events_async(
            backend,
            prompt=request.prompt,
            model=request.model,
            images=request.images or [],
    ...
    from fastapi.concurrency import run_in_threadpool
    image_urls = await run_in_threadpool(backend.resolve_conversation_image_urls, conversation_id, file_ids, sediment_ids)
    if image_urls:
        image_bytes = await run_in_threadpool(backend.download_image_bytes, image_urls)
        image_items = [
            {"b64_json": base64.b64encode(image_data).decode("ascii")}
...
async def stream_image_outputs_with_pool_async(request: ConversationRequest) -> AsyncIterator[ImageOutput]:
    if str(request.model or "").strip() not in IMAGE_MODELS:
        raise ImageGenerationError("unsupported image model,supported models: " + ", ".join(IMAGE_MODELS))
    ...
    for index in range(1, request.n + 1):
        while True:
            try:
                from fastapi.concurrency import run_in_threadpool
                token = await run_in_threadpool(account_service.get_available_access_token)
            except RuntimeError as exc:
                if emitted:
                    return
                raise ImageGenerationError(str(exc) or "image generation failed") from exc
            ...
            try:
                backend = OpenAIBackendAPI(access_token=token)
                async for output in stream_image_outputs_async(backend, request, index, request.n):
                    if output.kind == "message" and request.message_as_error:
                        raise ImageGenerationError(
                            output.text or "Image generation was rejected by upstream policy.",
                            status_code=400,
                ...
                if returned_message or not returned_result:
                    from fastapi.concurrency import run_in_threadpool
                    await run_in_threadpool(account_service.mark_image_result, token, False)
                    return
                from fastapi.concurrency import run_in_threadpool
                await run_in_threadpool(account_service.mark_image_result, token, True)
                break
            except ImageGenerationError:
                from fastapi.concurrency import run_in_threadpool
                await run_in_threadpool(account_service.mark_image_result, token, False)
                ...
                if not emitted_for_token and is_token_invalid_error(last_error):
                    from fastapi.concurrency import run_in_threadpool
                    await run_in_threadpool(account_service.remove_invalid_token, token, "image_stream")
                    continue
                raise ImageGenerationError(image_stream_error_message(last_error)) from exc
...
async def stream_image_chunks_async(outputs: AsyncIterable[ImageOutput]) -> AsyncIterator[dict[str, Any]]:
    async for output in outputs:
        yield output.to_chunk()

async def collect_image_outputs_async(outputs: AsyncIterable[ImageOutput]) -> dict[str, Any]:
    created = None
    data: list[dict[str, Any]] = []
    message = ""
    progress_parts: list[str] = []
