...
async def handle(body: dict[str, Any]) -> dict[str, Any] | AsyncIterator[dict[str, Any]]:
    if body.get("stream"):
        if is_image_chat_request(body):
            return image_chat_events_async(body)
        model, messages, tools, choice = text_chat_parts(body)
        if tools:
            return stream_text_chat_completion_with_tools_async(text_backend(), messages, model, tools, choice)
        return stream_text_chat_completion_async(text_backend(), messages, model)
    
    if is_image_chat_request(body):
        return await image_chat_response_async(body)
    
    model, messages, tools, choice = text_chat_parts(body)
    if not tools:
        request = ConversationRequest(model=model, messages=messages)
        text = await collect_text_async(text_backend(), request)
        return completion_response(model, text, messages=messages)
    ...
    if _required_violation(choice, tool_calls):
        retry_messages = normalize_messages(_required_retry_messages(messages, text, choice))
        retry_text = await collect_text_async(text_backend(), ConversationRequest(model=model, messages=retry_messages))
        retry_calls = _apply_choice_filter(_tool_calls_from_text(retry_text), choice)
        if retry_calls:
            tool_calls = retry_calls
            text = retry_text
...
async def stream_text_chat_completion_async(backend: OpenAIBackendAPI, messages: list[dict[str, Any]], model: str) -> AsyncIterator[dict[str, Any]]:
    completion_id = f"chatcmpl-{uuid.uuid4().hex}"
    created = int(time.time())
    yield completion_chunk(model, {"role": "assistant", "content": ""}, None, completion_id, created)
    async for delta in stream_text_deltas_async(backend, ConversationRequest(messages=messages, model=model)):
        yield completion_chunk(model, {"content": delta}, None, completion_id, created)
    yield completion_chunk(model, {}, "stop", completion_id, created)

async def stream_text_chat_completion_with_tools_async(backend: OpenAIBackendAPI, messages: list[dict[str, Any]], model: str, tools: list[dict[str, Any]], choice: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
    completion_id = f"chatcmpl-{uuid.uuid4().hex}"
    created = int(time.time())
    yield completion_chunk(model, {"role": "assistant", "content": ""}, None, completion_id, created)
    ...
    if _required_violation(choice, tool_calls):
        retry_messages = normalize_messages(_required_retry_messages(messages, text, choice))
        retry_parts = []
        async for delta in stream_text_deltas_async(backend, ConversationRequest(messages=retry_messages, model=model)):
        ...
        for idx, call in enumerate(tool_calls):
            chunk = {
                "tool_calls": [{
                    ...
                    "id": f"call_{uuid.uuid4().hex[:24]}",
                    "type": "function",
                    "function": {"name": call["name"], "arguments": call["args_text"]},
                }]
            }
            yield completion_chunk(model, chunk, None, completion_id, created)

    yield completion_chunk(model, {}, "tool_calls" if tool_calls else "stop", completion_id, created)

async def image_chat_response_async(body: dict[str, Any]) -> dict[str, Any]:
    model, prompt, n, images = chat_image_args(body)
    outputs = stream_image_outputs_with_pool_async(ConversationRequest(
        prompt=prompt,
        model=model,
        n=n,
        response_format="b64_json",
    ...
    result = await collect_image_outputs_async(outputs)
    return completion_response(model, image_result_content(result), int(result.get("created") or 0) or None)

async def image_chat_events_async(body: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
    model, prompt, n, images = chat_image_args(body)
    ...
    async for chunk in stream_image_chat_completion_async(image_outputs, model):
        yield chunk

async def stream_image_chat_completion_async(image_outputs: AsyncIterable[ImageOutput], model: str) -> AsyncIterator[dict[str, Any]]:
    completion_id = f"chatcmpl-{uuid.uuid4().hex}"
    created = int(time.time())
    sent_role = False
    sent_text = ""
