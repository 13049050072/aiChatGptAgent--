...
import time
import uuid
from typing import Any, Iterable, Iterator, AsyncIterable, AsyncIterator

from fastapi import HTTPException

from services.protocol.conversation import (
    stream_image_outputs_with_pool_async,
    stream_text_deltas_async,
    ConversationRequest,
    ImageOutput,
    encode_images,
...
def handle_sync(body: dict[str, Any]) -> dict[str, Any] | Iterator[dict[str, Any]]:
...
async def stream_text_response_async(backend: OpenAIBackendAPI, body: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
    prompt = extract_response_prompt(body.get("input"))
    model = str(body.get("model") or "auto").strip() or "auto"
    request = ConversationRequest(prompt=prompt, model=model)
    response_id = f"resp_{uuid.uuid4().hex}"
    created = int(time.time())
    yield response_created(response_id, model, created)
    ...
    if not fn_tools or choice.get("mode") == "none":
        text_id = f"item_{uuid.uuid4().hex}"
        yield {"type": "response.output_text.added", "item_id": text_id, "output_index": 0, "content_index": 0}
        
        parts = []
        async for delta in stream_text_deltas_async(backend, request):
            parts.append(delta)
            yield {"type": "response.output_text.delta", "item_id": text_id, "output_index": 0, "content_index": 0, "delta": delta}
            
        text = "".join(parts)
        yield {"type": "response.output_text.done", "item_id": text_id, "output_index": 0, "content_index": 0, "text": text}
        item = text_output_item(text, text_id)
        output_items.append(item)
        yield {"type": "response.output_item.done", "output_index": 0, "item": item}
        yield response_completed(response_id, model, created, output_items)
        return
    ...
    text_id = f"item_{uuid.uuid4().hex}"
    text_opened = False
    call_states: dict[int, dict[str, Any]] = {}
    ...
    async for delta in stream_text_deltas_async(backend, request):
        ...
        calls = _apply_choice_filter_response(calls, choice)
        ...
        if not calls:
            ...
            if text_opened and text_id not in [i["id"] for i in output_items]:
                # 关闭文本流
                stripped = _strip_tool_markup(text)
                yield {"type": "response.output_text.done", "item_id": text_id, "output_index": 0, "content_index": 0, "text": stripped}
                item = text_output_item(stripped, text_id)
                output_items.append(item)
                yield {"type": "response.output_item.done", "output_index": 0, "item": item}
                
            for idx, call in enumerate(calls):
                output_index = idx + (1 if text_opened else 0)
                canonical_name = call["name"]
                canonical_args = call["args_text"]
                ...
                state = call_states[idx]
                if not _args_semantically_equal(state["emitted_args"], canonical_args):
                    diff = canonical_args[len(state["emitted_args"]):] if canonical_args.startswith(state["emitted_args"]) else canonical_args[len(state["emitted_args"]):]
                    if diff:
                        yield {
                            "type": "response.function_call_arguments.delta",
                            "item_id": state["item_id"],
    ...
    if text_opened and not calls and text_id not in [i["id"] for i in output_items]:
        stripped = _strip_tool_markup(text)
        yield {"type": "response.output_text.done", "item_id": text_id, "output_index": 0, "content_index": 0, "text": stripped}
        item = text_output_item(stripped, text_id)
        output_items.append(item)
...
async def stream_image_response_async(image_outputs: AsyncIterable[ImageOutput], prompt: str, model: str) -> AsyncIterator[dict[str, Any]]:
    response_id = f"resp_{uuid.uuid4().hex}"
    created = int(time.time())
    yield response_created(response_id, model, created)
    async for output in image_outputs:
        if output.kind == "message":
            text = output.text
            item = text_output_item(text)
            yield {"type": "response.output_text.delta", "item_id": item["id"], "output_index": 0, "content_index": 0, "delta": text}
...
async def collect_response_async(events: AsyncIterable[dict[str, Any]]) -> dict[str, Any]:
    completed = {}
...
async def response_events_async(body: dict[str, Any]) -> AsyncIterator[dict[str, Any]]:
    if is_text_response_request(body):
        async for chunk in stream_text_response_async(text_backend(), body):
            yield chunk
        return
    ...
    if image_info:
        ...
        images = None
    image_outputs = stream_image_outputs_with_pool_async(ConversationRequest(
        prompt=prompt,
        model=model,
        size=None if images else "1:1",
        response_format="b64_json",
    ...
    async for chunk in stream_image_response_async(image_outputs, prompt, model):
        yield chunk

async def handle(body: dict[str, Any]) -> dict[str, Any] | AsyncIterator[dict[str, Any]]:
    if body.get("stream"):
        return response_events_async(body)
        
    response = await collect_response_async(response_events_async(body))
    ...
    retry_input.append({"type": "message", "role": "user", "content": nudge})
    retry_body["input"] = retry_input
    retry_response = await collect_response_async(response_events_async(retry_body))
    retry_output = retry_response.get("output") or []
    if any(isinstance(o, dict) and o.get("type") == "function_call" for o in retry_output):
        return retry_response
    return response
