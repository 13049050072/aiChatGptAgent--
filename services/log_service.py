...
from utils.helper import anthropic_sse_stream, sse_json_stream, sse_json_stream_async, anthropic_sse_stream_async
...
class LogService:
    ...
    def add(self, type: str, summary: str = "", detail: dict[str, Any] | None = None, **data: Any) -> None:
        ...
        try:
            ...
            loop.create_task(run_in_threadpool(_write))
        except RuntimeError:
            _write()
...
class LoggedCall:
    ...
    async def run(self, handler, *args, sse: str = "openai"):
        ...
        try:
            if inspect.iscoroutinefunction(handler) or (hasattr(handler, "func") and inspect.iscoroutinefunction(handler.func)):
        ...
        if isinstance(result, AsyncIterable):
            sender_async = anthropic_sse_stream_async if sse == "anthropic" else sse_json_stream_async
            try:
                # get first chunk
                async def _get_first(ait):
                    ait_iter = aiter(ait)
                    try:
                        first = await anext(ait_iter)
                        return True, first, ait_iter
                    except StopAsyncIteration:
                        return False, None, ait_iter

                has_first, first, result_iter = await _get_first(result)
            except ImageGenerationError as exc:
                self.log("调用失败", status="failed", error=str(exc))
                self._refund()
                return _image_error_response(exc)
            ...
            if not has_first:
                ...
                async def empty_gen():
                    yield None
                    return
                return StreamingResponse(sender_async(empty_gen()), media_type="text/event-stream")
            ...
            return StreamingResponse(sender_async(self.stream_async(async_chain())), media_type="text/event-stream")
            ...
            if not failed:
                ...
                if image_result_data:
                    ...
                    try:
                        ...
                        record_prompt_for_result(self.request_text, image_result_data, is_edit=is_edit)
                    except Exception:
                        pass

    async def stream_async(self, items):
        urls: list[str] = []
        image_result_data: list[dict[str, Any]] = []
        failed = False
        try:
