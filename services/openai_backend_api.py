...
class OpenAIBackendAPI:
    ...
    async def stream_conversation_async(
            self,
            messages: Optional[list[Dict[str, Any]]] = None,
            model: str = "auto",
            prompt: str = "",
        ...
        from utils.helper import iter_sse_payloads_async
        system_hints = system_hints or []
        if "picture_v2" in system_hints:
            async for item in self._stream_picture_conversation_async(prompt, model, images or []):
                yield item
            return

        normalized = messages or [{"role": "user", "content": prompt}]
        await run_in_threadpool(self._bootstrap)
        requirements = await run_in_threadpool(self._get_chat_requirements)
        path, timezone = await run_in_threadpool(self._chat_target)
        payload = await run_in_threadpool(self._conversation_payload, normalized, model, timezone)
        
        if not hasattr(self, 'async_session'):
            from curl_cffi.requests import AsyncSession
            self.async_session = AsyncSession(**proxy_settings.build_session_kwargs(
                impersonate=self.fp["impersonate"],
                verify=True,
            ))
            self.async_session.headers.update(self.session.headers)
        ...
        ensure_ok(response, path)
        try:
            async for item in iter_sse_payloads_async(response):
                yield item
        finally:
            # Note: curl_cffi AsyncSession.stream() does not return a response that has a close() method like sync. 
            # It's an async context manager typically, or if just post with stream=True, we can await response.aclose().
    ...
    async def _stream_picture_conversation_async(
            self,
            prompt: str,
            model: str,
            images: list[str],
        ...
        def _upload_all():
            return [self._upload_image(image, f"image_{idx}.png") for idx, image in enumerate(images, start=1)]
            
        references = await run_in_threadpool(_upload_all)
        await run_in_threadpool(self._bootstrap)
        requirements = await run_in_threadpool(self._get_chat_requirements)
        conduit_token = await run_in_threadpool(self._prepare_image_conversation, prompt, requirements, model)
        response = await self._start_image_generation_async(prompt, requirements, conduit_token, model, references)
        try:
            async for item in iter_sse_payloads_async(response):
                yield item
        finally:
    ...
    async def _start_image_generation_async(self, prompt: str, requirements: ChatRequirements, conduit_token: str, model: str, references: Optional[list[Dict[str, Any]]] = None) -> Any:
        references = references or []
        parts = [{
            "content_type": "image_asset_pointer",
            "asset_pointer": f"file-service://{item['file_id']}",
