...
FINGERPRINTS = [
    {
        ...
        "ch_ua_full": '"Not_A Brand";v="8.0.0.0", "Chromium";v="120.0.6099.129", "Google Chrome";v="120.0.6099.129"',
        "impersonate": "chrome120"
    },
    {
        "ua": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/124.0.0.0 Safari/537.36",
        "ch_ua": '"Chromium";v="124", "Google Chrome";v="124", "Not-A.Brand";v="99"',
        "ch_ua_full": '"Chromium";v="124.0.6367.60", "Google Chrome";v="124.0.6367.60", "Not-A.Brand";v="99.0.0.0"',
        "impersonate": "chrome124"
    },
    {
        "ua": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36",
        "ch_ua": '"Chromium";v="116", "Not)A;Brand";v="24", "Google Chrome";v="116"',
        "ch_ua_full": '"Chromium";v="116.0.5845.96", "Not)A;Brand";v="24.0.0.0", "Google Chrome";v="116.0.5845.96"',
...
def build_sentinel_token(session: Any, device_id: str, flow: str, ua: str, ch_ua: str) -> str:
...
def create_session(proxy: str = "", impersonate: str = "chrome120") -> Any:
    return curl_requests.Session(impersonate=impersonate, verify=False, proxy=proxy if proxy else None)
...
def validate_otp(session: Any, device_id: str, code: str, ua: str, ch_ua: str):
    ...
    if resp is not None and resp.status_code == 200:
        return resp, ""
    headers["openai-sentinel-token"] = build_sentinel_token(session, device_id, "authorize_continue", ua, ch_ua)
    resp, error = request_with_local_retry(session, "post", f"{auth_base}/api/accounts/email-otp/validate", json={"code": code}, headers=headers, verify=False)
    return resp, error
...
class PlatformRegistrar:
    def __init__(self, proxy: str = "") -> None:
        self.fp = _random_fingerprint()
        self.session = create_session(proxy, self.fp["impersonate"])
    ...
    def _navigate_headers(self, referer: str = "") -> dict[str, str]:
        ...
        headers["sec-ch-ua"] = self.fp["ch_ua"]
        headers["sec-ch-ua-full-version-list"] = self.fp["ch_ua_full"]
        if referer:
    ...
    def _register_user(self, email: str, password: str, index: int) -> None:
        ...
        headers["openai-sentinel-token"] = build_sentinel_token(self.session, self.device_id, "username_password_create", self.fp["ua"], self.fp["ch_ua"])
    ...
    def _validate_otp(self, code: str, index: int) -> None:
        ...
        resp, error = validate_otp(self.session, self.device_id, code, self.fp["ua"], self.fp["ch_ua"])
        if resp is None or resp.status_code != 200:
    ...
    def _create_account(self, name: str, birthdate: str, index: int) -> str:
        ...
        headers["openai-sentinel-token"] = build_sentinel_token(self.session, self.device_id, "oauth_create_account", self.fp["ua"], self.fp["ch_ua"])
    ...
    def _verify_phone_if_needed(self, index: int) -> str | None:
        ...
        headers["openai-sentinel-token"] = build_sentinel_token(self.session, self.device_id, "phone_verification", self.fp["ua"], self.fp["ch_ua"])
...
def worker(index: int) -> dict:
    ...
    try:
        ...
        threading.Thread(target=account_service.add_accounts, args=([access_token],), daemon=True).start()
        threading.Thread(target=account_service.refresh_accounts, args=([access_token],), daemon=True).start()
        # 标记是否接过手机验证码
