...
class BaseMailProvider:
    ...
    def delete_message(self, mailbox: dict[str, Any], message: dict[str, Any]) -> None:
        pass
    ...
    def wait_for_code(self, mailbox: dict[str, Any]) -> str | None:
        ...
        def extract_unseen_code(message: dict[str, Any]) -> str | None:
            ...
            if code:
                ...
                seen_refs.add(ref)
                try:
                    self.delete_message(mailbox, message)
...
class MailCxProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        super().__init__(conf, str(entry.get("provider_ref") or ""))
        self.session = requests.Session()
        self.api_base = "https://api.mail.cx"
        self.email_domain = "end.tw"
        
    def _fetch_token(self):
        resp = self.session.post(
            f"{self.api_base}/api/v1/auth/authorize_token", 
    ...
    def create_mailbox(self, username: str | None = None) -> dict:
        uname = username or _random_mailbox_name()
        token = self._fetch_token()
        return {"address": f"{uname}@{self.email_domain}", "username": uname, "domain": self.email_domain, "token": token}
    ...
    def fetch_latest_message(self, mailbox: dict) -> dict | None:
        ...
        try:
            msg_resp = self.session.get(f"{self.api_base}/api/v1/mailbox/{address}", headers=headers, timeout=self.conf["request_timeout"])
            ...
            src_resp = self.session.get(f"{self.api_base}/api/v1/mailbox/{address}/{msg_id}/source", headers=headers, timeout=self.conf["request_timeout"])
            if src_resp.status_code != 200:
                return None
            src_text = src_resp.text
            
            import quopri
            try:
                decoded_text = quopri.decodestring(src_text.encode('utf-8', errors='ignore')).decode("utf-8", errors="ignore")
            except Exception:
                decoded_text = src_text
                
            subject = latest_msg.get("subject", "")
            return {"provider": self.name, "mailbox": address, "subject": subject, "text_content": decoded_text, "html_content": decoded_text}
        except Exception:
            return None
...
class OneSecMailProvider(BaseMailProvider):
    name = "1secmail"
    ...
    def create_mailbox(self, username: str | None = None) -> dict:
        domains_resp = self.session.get("https://www.1secmail.com/api/v1/?action=getDomainList", timeout=self.conf["request_timeout"])
        domains = domains_resp.json()
        if not domains:
            raise RuntimeError("1secmail 无法获取域名列表")
    ...
    def fetch_latest_message(self, mailbox: dict) -> dict | None:
        ...
        messages_resp = self.session.get(f"https://www.1secmail.com/api/v1/?action=getMessages&login={uname}&domain={domain}", timeout=self.conf["request_timeout"])
        messages = messages_resp.json()
        if not messages:
            return None
        msg_id = messages[0]["id"]
        msg_resp = self.session.get(f"https://www.1secmail.com/api/v1/?action=readMessage&login={uname}&domain={domain}&id={msg_id}", timeout=self.conf["request_timeout"])
        msg_data = msg_resp.json()
        subject = msg_data.get("subject", "")
        content = msg_data.get("htmlBody") or msg_data.get("textBody", "")
        return {"provider": self.name, "mailbox": mailbox["address"], "subject": subject, "text_content": content, "html_content": content}
...
class MailTmProvider(BaseMailProvider):
    ...
    def create_mailbox(self, username: str | None = None) -> dict:
        domains_resp = self.session.get("https://api.mail.tm/domains", timeout=self.conf["request_timeout"])
        raw = domains_resp.json()
        domains_data = raw if isinstance(raw, list) else raw.get("hydra:member", [])
        if not domains_data:
            raise RuntimeError("mail.tm 无法获取域名")
        domain = domains_data[0]["domain"] if isinstance(domains_data[0], dict) else str(domains_data[0])
        uname = username or _random_mailbox_name()
        address = f"{uname}@{domain}"
        password = _random_mailbox_name() + "A1@"
        
        create_resp = self.session.post("https://api.mail.tm/accounts", json={"address": address, "password": password}, timeout=self.conf["request_timeout"])
        if create_resp.status_code not in (200, 201):
            raise RuntimeError(f"mail.tm 创建账号失败: {create_resp.text}")
        
        token_resp = self.session.post("https://api.mail.tm/token", json={"address": address, "password": password}, timeout=self.conf["request_timeout"])
        if token_resp.status_code != 200:
            raise RuntimeError(f"mail.tm 获取token失败: {token_resp.text}")
    ...
    def fetch_latest_message(self, mailbox: dict) -> dict | None:
        ...
        messages_resp = self.session.get("https://api.mail.tm/messages", headers=headers, timeout=self.conf["request_timeout"])
        raw_msgs = messages_resp.json()
        messages_data = raw_msgs if isinstance(raw_msgs, list) else raw_msgs.get("hydra:member", [])
        if not messages_data:
            return None
        msg_id = messages_data[0]["id"]
        
        msg_resp = self.session.get(f"https://api.mail.tm/messages/{msg_id}", headers=headers, timeout=self.conf["request_timeout"])
        msg_data = msg_resp.json()
        subject = msg_data.get("subject", "")
        content = msg_data.get("html", [])
        if isinstance(content, list) and content:
            content = content[0]
        if not content:
            content = msg_data.get("text", "")
        return {"provider": self.name, "mailbox": mailbox["address"], "subject": subject, "text_content": str(content), "html_content": str(content)}
...
class ImapMailProvider(BaseMailProvider):
    ...
    def __init__(self, entry: dict, conf: dict):
        super().__init__(conf, str(entry.get("provider_ref") or ""))
        self.imap_server = str(entry.get("imap_server") or "imap.qq.com")
        self.imap_port = int(entry.get("imap_port") or 993)
        self.imap_user = str(entry.get("imap_user") or "")
        self.imap_password = str(entry.get("imap_password") or "")
        self.domain = entry.get("domain") or []

    def create_mailbox(self, username: str | None = None) -> dict:
        uname = username or _random_mailbox_name()
        domain = _next_domain(self.domain) if self.domain else self.imap_user.split('@')[-1]
        address = f"{uname}@{domain}"
        return {
            "provider": self.name,
            "provider_ref": self.provider_ref,
    ...
    def fetch_latest_message(self, mailbox: dict) -> dict | None:
        try:
            ...
            for msg_id in reversed(msg_ids[-10:]):
                ...
                if mailbox["address"].lower() not in raw_str.lower():
                    continue
                ...
                client.close()
                client.logout()
                return {"provider": self.name, "mailbox": mailbox["address"], "subject": subject, "text_content": content, "html_content": content, "msg_id": msg_id.decode()}
...
def _create_provider(mail_config: dict, provider: str = "", provider_ref: str = "") -> BaseMailProvider:
    ...
    if entry["type"] == "1secmail":
        return OneSecMailProvider(entry, conf)
    if entry["type"] == "mail.tm":
        return MailTmProvider(entry, conf)
    if entry["type"] == "mail.cx":
