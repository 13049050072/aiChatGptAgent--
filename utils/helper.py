...
async def sse_json_stream_async(items) -> Any:
    yield ": stream-open\n\n"
    try:
...
async def anthropic_sse_stream_async(items) -> Any:
    try:
        async for item in items:
            event = str(item.get("type") or "message_delta") if isinstance(item, dict) else "message_delta"
            yield f"event: {event}\n"
        ...
        logger.warning({
            "event": "anthropic_sse_stream_error_async",
            "error_type": exc.__class__.__name__,
            "error": str(exc),
        })
        error = {"type": "error", "error": {"type": exc.__class__.__name__, "message": str(exc)}}
...
async def iter_sse_payloads_async(response) -> Any:
